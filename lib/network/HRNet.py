...
class HRNet(nn.Module):
    def __init__(self, cfg, input_channal, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(input_channal, 64, kernel_size=4, stride=2, padding=1, bias=False)
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i+1-num_branches_pre):
                    ...
                    conv3x3s.append(nn.Sequential(
                        Conv2d(
                            inchannels, outchannels, 4, 2, 1, bias=False),
    ...
    def get_loss(self, data):
        ...
        loss = F.l1_loss(
            torch.masked_select(pr, data["mask"]),
            torch.masked_select(data["ceof"], data["mask"])
        )
        return loss, {"loss":loss.detach()}

def get_HRNet(input_channal = 38):
    cfg = {
        "STAGE1":{
            "NUM_MODULES": 1,
            "NUM_BRANCHES": 1,
